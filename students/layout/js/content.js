...
const saveChanges = function (cell) {
    return () => {
        if (cell.getAttribute('dirty') == "true") {
            ...
            data.id = cell.parentElement.childNodes[0].textContent;
...
// More Options Form Show Hide
let moreOpsBtn = document.querySelector('.more-options span.show-hide');
moreOpsBtn.addEventListener('click', () => {
    document.querySelector('.more-options form').classList.toggle('hidden');
});
